...
def test_friction_severity_expr_precedence_and_all_branches(valid_severity_config):
    ...
    result = df.with_columns(fsm.friction_severity_expr(valid_severity_config))[
        "friction_severity_class"
    ].to_list()
    assert result == [
...
def test_load_cfpb_with_severity_attaches_taxonomy_bucket_when_mapping_provided(
    tmp_path, valid_severity_config
):
    cfpb_path = tmp_path / "cfpb.csv"
    pl.DataFrame(
        [
...
def test_load_cfpb_with_severity_no_taxonomy_bucket_column_when_mapping_omitted(
    tmp_path, valid_severity_config
):
    cfpb_path = tmp_path / "cfpb.csv"
